...
def apply_style():
    style_path = Path(__file__).parent / '../resources/figure_stylesheet.mplstyle'
...
def plot_sem_tube(
    ...
    *,
    sem_x=None,
    sem_y=None,
    cov=None,                 # shape (N,2,2) if provided
    k=1.0,                    # 1=±1 SEM; 1.96≈95% CI (if SEM is converted to CI)
    ax=None,
    color=None,               # matplotlib color (None = use default cycle)
    alpha=0.25,
    linewidth=2.0,
    closed=False,             # True for closed loops
    rounded_caps=True,        # rounded end caps for open curves
    cap_points=24,            # resolution of rounded caps
    plot_mean=True,           # also draw the mean line
):
    """
    Plot a smooth 2D SEM/CI 'tube' around the (x_mean, y_mean) trajectory.

    Parameters
    ----------
    x_mean, y_mean : array-like, shape (N,)
        Mean trajectory coordinates (e.g., PC1 vs PC2 means).
    sem_x, sem_y : array-like, shape (N,), optional
        Per-axis SEM for x and y. Required if 'cov' is not provided.
    cov : array-like, shape (N,2,2), optional
        Full covariance per point. If provided, overrides sem_x/sem_y.
    k : float
        Multiplier for the band radius (1=SEM, 1.96≈95% CI, etc.).
    ...
    linewidth : float
        Mean line width.
    closed : bool
        Treat trajectory as closed (periodic tangents, no end caps).
    rounded_caps : bool
        Add rounded end caps for open curves.
    cap_points : int
        Resolution of the rounded caps.
    plot_mean : bool
        Also draw the mean trajectory line.
    ...
    N = len(x)
    assert y.shape == x.shape and x.ndim == 1 and N >= 2, "x/y must be 1D and same length"

    if cov is None:
        assert sem_x is not None and sem_y is not None, "Provide sem_x & sem_y or cov"
        sem_x = np.asarray(sem_x, dtype=float)
        sem_y = np.asarray(sem_y, dtype=float)
        assert sem_x.shape == x.shape and sem_y.shape == x.shape, "SEM arrays must match x/y length"
    else:
        cov = np.asarray(cov, dtype=float)
        assert cov.shape == (N, 2, 2), "cov must be (N,2,2)"

    # --- tangents and unit normals ---
    if closed and N > 2:
    ...
    tnorm[tnorm == 0] = 1.0
    # left-hand normal (rotate tangent by +90°)
    nx = -dy / tnorm
    ny =  dx / tnorm

    # --- radius in the local normal direction ---
    if cov is not None:
        # r_i = k * sqrt(n^T Σ n)
        n = np.stack([nx, ny], axis=1)                     # (N,2)
        r = k * np.sqrt(np.einsum('ni,nij,nj->n', n, cov, n))
    else:
        r = k * np.sqrt((nx * sem_x)**2 + (ny * sem_y)**2)

    # Offset curves (outer = +n, inner = -n)
    x_out, y_out = x + r * nx, y + r * ny
    x_in , y_in  = x - r * nx, y - r * ny

    # --- build polygon (optionally with rounded caps) ---
    def _cap(center_x, center_y, n_x, n_y, radius, start_to_inner=True):
        """Half-circle cap centered at (x,y), oriented by normal vector."""
        ang = np.arctan2(n_y, n_x)
        if start_to_inner:
            theta = np.linspace(ang + np.pi/2, ang - np.pi/2, cap_points)
        else:
            theta = np.linspace(ang - np.pi/2, ang + np.pi/2, cap_points)
        return center_x + radius * np.cos(theta), center_y + radius * np.sin(theta)

    if closed:
        xp = np.concatenate([x_out, x_in[::-1]])
        yp = np.concatenate([y_out, y_in[::-1]])
    else:
        if rounded_caps:
            # start cap (outer -> inner)
            cx0, cy0, r0 = x[0], y[0], r[0]
            cap0x, cap0y = _cap(cx0, cy0, nx[0], ny[0], r0, start_to_inner=True)
            # end cap (inner -> outer)
            cx1, cy1, r1 = x[-1], y[-1], r[-1]
            cap1x, cap1y = _cap(cx1, cy1, nx[-1], ny[-1], r1, start_to_inner=False)

            xp = np.concatenate([x_out, cap1x, x_in[::-1], cap0x])
            yp = np.concatenate([y_out, cap1y, y_in[::-1], cap0y])
        else:
            xp = np.concatenate([x_out, x_in[::-1]])
            yp = np.concatenate([y_out, y_in[::-1]])
    ...
    if ax is None:
        ax = plt.gca()
    fill_poly = ax.fill(xp, yp, alpha=alpha, fc=color, linewidth=0)[0]
    mean_line = None
    if plot_mean:
        mean_line, = ax.plot(x, y, color=color, lw=linewidth)

    # nice defaults
    ax.set_aspect("equal", adjustable="datalim")
    ax.autoscale(enable=True, tight=False)

    return fill_poly, mean_line
