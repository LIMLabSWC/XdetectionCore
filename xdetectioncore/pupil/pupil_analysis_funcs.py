...
from ..behaviour import sync_beh2sound, get_all_cond_filts, add_datetimecol, \
...
class PupilCondAnalysis:
    ...
    def plot_diff_ts_by_cond(self, diff_conditions=None, cond_line_kwargs=None, group_level='sess',
                             plot=None, plot_kwargs=None):
        """
        ...
        plot = plot_pupil_diff_ts_by_cond(
            ...
            cond_line_kwargs=cond_line_kwargs, group_level=group_level, plot_kwargs=plot_kwargs, plot=plot
        )
    ...
    def shuffle_by_cond(self, n_permutations=500, rng_seed=None):
        ...
        for sess in unique_sess:
            ...
            for cond_i, cond in enumerate(resps_by_cond):
                perm_resps = [shuff[cond_i].astype(np.float32) for shuff in shuffled_resps_by_cond]
...
def run_pupil_cond_analysis(
    ...
    if n_permutations is not None:
        for comp_i,comp_name in enumerate(analysis.cluster_analysis):
            ...
            obsv_diff = np.sum(analysis.cluster_analysis[comp_name][2])
            cluster_mass_ttest = ttest_1samp([np.sum(shuff) for shuff in cluster_mass_from_shuff],obsv_diff,
                                             alternative='less')
            cluster_mass_perm_test = (np.array([np.sum(shuff) for shuff in cluster_mass_from_shuff])>=obsv_diff).mean()
            ...
            stats_output = f'comp_name: {comp_name}. obsv diff: {obsv_diff}. n shuffles; {len(cluster_mass_from_shuff)} perm pval: {cluster_mass_perm_test}. ' \
                           f'range : {np.percentile(np.array([np.sum(shuff) for shuff in cluster_mass_from_shuff]), [0, 100])}'
            with open(tex_fp.parent / f'{tex_fp.stem.split("data_vs_shuffle")[0]}_{comp_name}_data_vs_shuffle_perm_stats.txt', 'w') as f:
