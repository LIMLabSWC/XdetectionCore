...
def load_pupil_data(pupil_data_path: Path | str, parent_dir=None,sess_list=None):
    """
    ...
    for sess in list(pupil_data.keys()):
        ...
        if sess_list:
            ...
            if not any(['_'.join([name,date]) in s for s in sess_list]) and sess in pupil_data.keys():
                pupil_data.pop(sess)
    return pupil_data
...
class LazyPupilLoader:
    ...
    def __getitem__(self, key):
        ...
        # 2. Wrap it in an object so you can call .pupildf
        # SimpleNamespace is a flexible object that allows dot notation
        return SimpleNamespace(pupildf=df)
