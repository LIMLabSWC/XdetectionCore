...
def read_lick_times(beh_event_path):
    beh_events = pd.read_csv(beh_event_path)
    return beh_events.query('Payload == 0')['Timestamp'].values
...
def get_drug_dates(dates_by_drug:list,):
    raise NotImplementedError('This function is deprecated. Use parse_drug_dates instead.')
...
def get_all_cond_filts():
    all_dict = dict(
        ...
        AB='Stage in [3,4,5] & Trial_Outcome==1 & N_TonesPlayed==2 & earlyX==True',
        ABC='Stage in [3,4,5] & Trial_Outcome==1 & N_TonesPlayed==3 & earlyX==True',
        ABCD='Stage in [3,4,5] & Trial_Outcome==1 & N_TonesPlayed==4 & earlyX==True',
    ...
    # Add early late to rare
    rare_freq_early_late_dict = {'early': '<col> <=5', 'late': '<col> >5'}
    for tag in list(rare_freq_early_late_dict.keys()):
        all_dict[f'rare_prate_{tag}'] = ' & '.join([all_dict['rare_prate'],
                                                    rare_freq_early_late_dict[tag].replace('<col>','rare_prate_cumsum')])
...
def group_td_df_across_sessions(sessions_objs:dict,sessnames:list) -> pd.DataFrame:
    ...
    for sessname in sessnames:
        ...
        if not sessions_objs[sessname].__class__.__name__ == 'Session':
            raise TypeError(f"Object for {sessname} is not a Session instance.")
...
def format_timestr(timestr_series,date=None) -> tuple[pd.Series, pd.Series]:
    """
...
def get_main_sess_patterns(name='', date='', main_sess_td_name='', home_dir=Path(''),td_df=None) -> list[int,int,int,int]:
...
def get_main_sess_td_df(_name=None, _date=None, _main_sess_td_name=None, _home_dir=None):
    ...
    [add_datetimecol(main_sess_td, col) for col in times2process
     if col in main_sess_td.columns]
